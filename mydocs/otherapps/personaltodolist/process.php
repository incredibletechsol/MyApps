...
<?php 
	if (isset($_GET['insert_description'])) {
		$desc=$_GET['insert_description'];
  		...
  		if(mysqli_query($dbconfig,"INSERT INTO tbl_todo_personal(description) values('$desc')")){
 ...
 ?>
  ...
  <?php 
    ...
    $delete_id=$_GET['delete_id'];
      if(mysqli_query($dbconfig,"DELETE FROM tbl_todo_personal WHERE id=$delete_id")){
 ...
 <?php 
    ...
    $new_desc= $_GET['new_desc'];
      if(mysqli_query($dbconfig,"UPDATE tbl_todo_personal SET description='$new_desc' WHERE id=$edit_id")){
